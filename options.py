...
# TODO: covered call, protective/married put, collar, strangle,
...
class Call(object):
    def __init__(self, K=None, price=None, St=None, pos='long'):
        ...
        self.pos = pos
    def payoff(self):
        return _sign[self.pos] * np.maximum(0., self.St - self.K)
    def profit(self):
        return self.payoff() - _sign[self.pos] * self.price
    def __repr__(self):
...
class Put(object):
    def __init__(self, K=None, price=None, St=None, pos='long'):
        ...
        self.pos = pos
    def payoff(self):
        return _sign[self.pos] * np.maximum(0., self.K - self.St)
    def profit(self):
        return self.payoff() - _sign[self.pos] * self.price
    def __repr__(self):
...
class OpStrat(object):
    ...
    def summary(self):
        if self.options:            
            payoffs = [op.payoff() for op in self.options]
            profits = [op.profit() for op in self.options]
            strikes = [op.K for op in self.options]
            prices = [op.price for op in self.options]
            exprs = [op.St for op in self.options]
            ...
            return None
    def payoff(self):
        return np.sum([op.payoff() for op in self.options], axis=0)
    def profit(self):
        return np.sum([op.profit() for op in self.options], axis=0)
...
class BullSpread(OpStrat):
    def __init__(self, St=None, K1=None, K2=None, price1=None, price2=None, 
        ...
        if kind == 'call':
            # Long call with K1/price1, short call with K2/price2
            self.add_option(K=K1, price=price1, St=St, kind='call', pos='long')
            self.add_option(K=K2, price=price2, St=St, kind='call', 
                            pos='short')
...
class BearSpread(OpStrat):
    ...
    def __init__(self, St=None, K1=None, K2=None, price1=None, price2=None, 
        ...
        if kind == 'call':
            self.add_option(K=K1, price=price1, St=St, kind='call', 
                            pos='short')
            self.add_option(K=K2, price=price2, St=St, kind='call', pos='long')

        if kind == 'put':
            # Short put with K1/price1, long put with K2/price2
