...
from pyfinance import ols, returns, utils

InFormat = Literal["num", "dec"]
EwmParam = Literal["alpha", "com", "span", "halflife"]


class EwmParams(TypedDict):
    """All four equivalent parameterizations of an EWM weight scheme."""
...
def activeshare(fund, idx, in_format: InFormat = "num"):
    """Compute the active ahare of a fund versus an index.
    ...
    # Resulting active share is a scalar when fund is 1-d.
    diff = fund - idx if fund.ndim == 1 else fund - idx[:, None]
...
def amortize(rate, nper, pv, freq="M"):  # pragma: no cover
    # `np.ppmt`/`np.ipmt`/`np.pmt` were removed in NumPy 1.20. This
    # function will raise AttributeError until reworked against
    # numpy_financial or an equivalent. Tracked for 2.1.
    """Construct an amortization schedule for a fixed-rate loan.
        ...
        with warnings.catch_warnings():
            ...
            for dist in self.distributions:
                ...
                try:
                    ...
                    pdf = dist.pdf(bin_edges, *shape, loc=loc, scale=scale)
...
def corr_heatmap(  # pragma: no cover
    x,
    mask_half=True,
    cmap="RdYlGn_r",
    vmin=-1,
...
def ewm_params(param: EwmParam, param_value: float) -> EwmParams:
    ...
    -------
    result : EwmParams
        Dict-like with all four equivalent parameterizations.
    """
...
def ewm_bootstrap(a, size=None, com=None, span=None, halflife=None, alpha=None):
    """Bootstrap a new distribution through exponential weighting.
    ...
    if not any((com, span, halflife, alpha)):
        raise ValueError("Specify one of `com`, `span`, `halflife`, `alpha`.")
    p = ewm_weights(i=len(a), com=com, span=span, halflife=halflife, alpha=alpha)
    return np.random.choice(a=a, size=size, p=p)


def factor_loadings(r, factors, scale=False):  # pragma: no cover
    # This function has accumulated latent bugs (it passes `hasconst` to
    # `OLS` whose parameter is `has_const`; it calls `model.beta()`
    # where `beta` is a property; etc.). Fix requires a broader rewrite.
    # Tracked for 2.1.
    """Security factor exposures generated through OLS regression.
    ...
    factors : DataFrame
        Factor returns (right-hand-side variables). Must contain at minimum
        an "RF" column and whichever factor columns the underlying models
        reference (see the factor-model registry in this function).
...
class PCA:
    ...
    def eigen_table(self):
        ...
        return pd.DataFrame(
            np.array([self.eigenvalues, self.inertia, self.cumulative_inertia]),
            columns=[f"F{i}" for i in range(1, self.keep + 1)],
            index=idx,
        )
    ...
    def loadings(self):
        ...
        cols = [f"PC{i}" for i in range(1, self.keep + 1)]
        return pd.DataFrame(loadings, columns=cols, index=self.feature_names)
    ...
    def varimax(self):
        ...
        for _i in range(q):
            ...
            u, s, vh = np.linalg.svd(
                np.dot(
                    loadings.T,
                    np.asarray(lam) ** 3
                    - (gamma / p) * np.dot(lam, np.diag(np.diag(np.dot(lam.T, lam)))),
                )
            )
            r = np.dot(u, vh)
            d = np.sum(s)
        ...
        return pd.DataFrame(np.dot(loadings, r), columns=cols, index=self.feature_names)


class PortSim:  # pragma: no cover
    # Flagged as work-in-progress in the original 2017 docstring below;
    # no downstream consumer in this repo exercises it. Treated as
    # experimental and excluded from coverage pending a rewrite.
    """Basic portfolio simulation.
    ...
    def __init__(
        ...
        if any((start, end)) and not lookback:
            ...
            raise ValueError(
                "if `lookback` is specified, both `start` and `end` should be None"
            )
        ...
        self.net = pd.DataFrame(self.net, index=self.index, columns=self.columns)
...
class TEOpt:
    ...
    def optimize(self):
        ...
        def te(weights, r, proxies):
            ...
            return np.std(proxy - r)  # not anlzd...
        ...
        xs = []
        funs = []
        for i, j in zip(self._r, self._proxies, strict=False):
            opt = sco.minimize(
                te,
                x0=ew,
                args=(i, j),
    ...
    def replicate(self):
        ...
        return np.sum(self.proxies[self.window :] * self._xs[:-1], axis=1).reindex(
            self.r.index
        )
