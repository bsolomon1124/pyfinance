...
__all__ = ["OLS", "PandasRollingOLS", "RegressionResult", "RollingOLS"]
...
class RegressionResult(Protocol):
    """Structural interface shared by `OLS`, `RollingOLS`, and `PandasRollingOLS`.

    Lets callers write code that accepts any of the three result types
    without importing the concrete classes. Only the common read-only
    fit attributes are part of the protocol; rolling-specific fields
    (windowed arrays, per-window indices, etc.) are not.
    """
    ...
    @property
    def rsq_adj(self) -> Any: ...

    @property
    def fstat(self) -> Any: ...

    @property
    def std_err(self) -> Any: ...
...
def _check_constant_params(a, has_const=False, use_const=True, rtol=1e-05, atol=1e-08):
    ...
    if all((has_const, use_const)):
        ...
        try:
            ...
            raise ValueError(
                "X data already contains a constant; please specify has_const=True"
            ) from e
        k = a.shape[-1] - 1
    else:
...
class RollingOLS:
    ...
    def _durbin_watson(self):
        return np.sum(
            np.square(np.diff(self._resids)) / np.expand_dims(self._ss_err, axis=-1),
            axis=1,
        )
...
class PandasRollingOLS(RollingOLS):
        ...
        if not names:
            if x is None:
                if hasattr(y, "columns"):
                    names = y.columns[1:-1] if has_const else y.columns[1:]
                else:
                    k = y.shape[-1] - 2 if has_const else y.shape[-1] - 1
                    names = [f"feature{i}" for i in range(1, k + 1)]
            else:
                if hasattr(x, "columns"):
                    names = x.columns[:-1] if has_const else x.columns
                else:
                    if has_const:
                        k = x.shape[-1] - 1
                    else:
    ...
    def _wrap_dataframe(self, stat):
        return DataFrame(getattr(self, stat), index=self.ridx, columns=self.names)

    def _wrap_multidx(self, stat, name=None):
        if name is None:
            name = stat[1:]
        outer = np.repeat(self.ridx, self.window)
        inner = np.ravel(utils.rolling_windows(self.index.values, window=self.window))
        return Series(
