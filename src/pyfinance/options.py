...
class BSM:
    ...
    def __init__(self, S0, K, T, r, sigma, kind: OptionKind = "call") -> None:
        kind = kind.lower()
    ...
    def gamma(self):
        return norm.pdf(self.d1, 0.0, 1.0) / (self.S0 * self.sigma * np.sqrt(self.T))
    ...
    def theta(self):
        return -1.0 * (self.S0 * norm.pdf(self.d1, 0.0, 1.0) * self.sigma) / (
            2.0 * np.sqrt(self.T)
        ) - 1.0 * self._sign[1] * self.r * self.K * np.exp(-self.r * self.T) * norm.cdf(
            self._sign[1] * self.d2, 0.0, 1.0
        )
    ...
    def omega(self):
        return (norm.cdf(self.d1, 0.0, 1.0) + self._sign[0]) * self.S0 / self.value()
...
class Option:
    ...
    def __repr__(self):
        if self.St is None:
            return f"{self.kind.title()}(K={self.K:0.2f}, price={self.price:0.2f}, St=None)"
        return f"{self.kind.title()}(K={self.K:0.2f}, price={self.price:0.2f}, St={self.price:0.2f})"
...
class Strangle(OpStrat):
    ...
    def __init__(self, St=None, K1=None, K2=None, callprice=None, putprice=None):
        super().__init__(St=St)
        self.K1 = K1
        self.K2 = K2
...
class BearSpread(OpStrat):
    ...
    def __init__(self, St=None, K1=None, K2=None, price1=None, price2=None, kind="put"):
        super().__init__(St=St)
        self.K2 = K1
        self.K2 = K2
        self.price1 = price1
...
class ShortPutLadder(BearSpread):
    ...
    def __init__(
        ...
        super().__init__(St=St, K1=K2, K2=K3, price1=price2, price2=price3, kind="put")
        self.K1 = K1
        self.price1 = price1
...
class _Butterfly(OpStrat):
    def __init__(
        ...
        if not np.allclose(np.mean([K1, K3]), K2):
            warnings.warn("specified strikes are not equidistant.", stacklevel=2)
        super().__init__(St=St)
        self.K1 = K1
        self.K2 = K2
        self.K3 = K3
...
class _Condor(OpStrat):
    def __init__(
        ...
        if not np.allclose(K2 - K1, K4 - K3):
            warnings.warn("specified wings are not equidistant.", stacklevel=2)
        super().__init__(St=St)
        self.K1 = K1
        self.K2 = K2
        self.K3 = K3
