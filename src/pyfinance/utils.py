...
def constrain_horizon(
    ...
    if strict and start < r.index[0]:
        raise ValueError(f"`start` pre-dates first element of the Index, {r.index[0]}")
    return r[start:end]
...
def encode(*args):
    ...
    for vec, s in zip(feature_vectors, args, strict=False):
        for word in s:
            vec[unique[word]] = 1
...
def expanding_stdize(obj, **kwargs):
    ...
    **kwargs -> passed to `obj.expanding`
    ...
    return (obj - obj.expanding(**kwargs).mean()) / (obj.expanding(**kwargs).std())
...
# Keyed by the integer value of each pandas `FreqGroup`. In pandas 2.x
# `FreqGroup` is a plain `Enum` (not `IntEnum`), so a dict keyed by the
# enum members can't be looked up with a raw int; we normalize to `.value`
# at both dict-construction and lookup time.
PERIODS_PER_YEAR = {
    FreqGroup.FR_ANN.value: 1.0,
    FreqGroup.FR_QTR.value: 4.0,
    FreqGroup.FR_MTH.value: 12.0,
    FreqGroup.FR_WK.value: 52.0,
    FreqGroup.FR_BUS.value: 252.0,
    FreqGroup.FR_DAY.value: 252.0,  # All days are business days
    FreqGroup.FR_HR.value: 252.0 * 6.5,
    FreqGroup.FR_MIN.value: 252.0 * 6.5 * 60,
    FreqGroup.FR_SEC.value: 252.0 * 6.5 * 60 * 60,
    FreqGroup.FR_MS.value: 252.0 * 6.5 * 60 * 60,
    FreqGroup.FR_US.value: 252.0 * 6.5 * 60 * 60 * 1000,
    FreqGroup.FR_NS.value: 252.0 * 6.5 * 60 * 60 * 1000 * 1000,  # someday...
}
...
def get_anlz_factor(freq):
    ...
    # Pass-through: callers sometimes have already-resolved numeric
    # annualization factors (e.g., from `_try_get_freq` on an inferred
    # period). Return it as-is rather than trying to re-parse.
    ...
    try:
        ...
        if freq.startswith(("A-", "BA-", "AS-", "BAS-")):
            ...
            raise ValueError(f"Invalid frequency: {freq}") from None
        base, mult = get_freq_code(freq)
    ...
    if type_:
        if isinstance(type_, str):
            if type_ in ["callable", "func", "function"]:
                ...
                type_ = [
                    i if i not in ["callable", "func", "function"] else Callable
                    for i in type_
                ]
        if isinstance(type_, str):
            # 'str' --> str (class `type`)
...
def random_weights(size, sumto=1.0):
    ...
    if w.ndim == 2:
        ...
        raise ValueError(f"`w.ndim` must be 1 or 2, not {w.ndim}")
    return w
...
def rolling_windows(a, window):
    ...
    if window > a.shape[0]:
        raise ValueError(
            f"Specified `window` length of {window} exceeds length of"
            f" `a`, {a.shape[0]}."
        )
...
def uniqify(seq):
    ...
    # Dict preserves insertion order as of Python 3.7 (Raymond Hettinger).
