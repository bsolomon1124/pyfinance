...
__all__ = ['appender', 'avail', 'can_broadcast', 'convertfreq', 'constrain', 
           ...
           'ewm_params', 'expanding_stdize', 'public_dir',
...
def convertfreq(freq):
    ...
    freqs.update(zip(weekoffsets, [52.] * len(annoffsets)))
    freqs.update(zip(qtroffsets, [4.] * len(annoffsets)))
...
def random_tickers(length, n, ends_in=None):
    ...
    if ends_in is None:
        tickers = npr.choice(letters, size=(n, length)).tolist()
        tickers = [''.join(inner) for inner in tickers]

    else:
        tickers = npr.choice(letters, size=(n, length-1)).tolist()
        tickers = [''.join(inner) for inner in tickers]
